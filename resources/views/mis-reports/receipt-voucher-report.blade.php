...
<table class="comp-det" style="width:100%;">
		<tr>
			...
			<td>
                ...
                <h4>Receipt Voucher Report<br> From {{\Carbon\Carbon::parse($start)->format('d-m-Y')}} to {{ \Carbon\Carbon::parse($end)->format('d-m-Y') }}</h4>
				</div>
...
<table border="0" style="width:100%;border-collapse:collapse;">
	...
	<tbody>
		...
		<tr>
            ...
            <td style="padding:3px;">{{ $receipt_detail->voucher_no }}</td>
            <td style="padding:3px;">{{ \Carbon\Carbon::parse($receipt_detail->payment_rcv_date)->format('d-m-Y') }}</td>
            <td style="padding:3px 3px 3px 3%;">{{ $debt_act }}</td>
			<td style="padding:3px;" class="center">{{ $receipt_detail->amount }}</td>
            <td style="padding:3px;" class="center"></td>
            @elseif($receipt_detail->payment_release_date == '0000-00-00' && $receipt_detail->payment_mode == 'cash')
            <td style="padding:3px;"></td>
            <td style="padding:3px;"></td>
            <td style="padding:3px 3px 3px 3%;">{{ $crdt_act }} <br> {{ $receipt_detail->narration }}</td>
			<td style="padding:3px;" class="center"></td>
            <td style="padding:3px;" class="center">{{ $receipt_detail->amount }}</td>
            @elseif($receipt_detail->voucher_date == '0000-00-00' && $receipt_detail->payment_mode != 'cash')
            <td style="padding:3px;">{{ $receipt_detail->voucher_no }}</td>
            <td style="padding:3px;">{{ \Carbon\Carbon::parse($receipt_detail->payment_rcv_date)->format('d-m-Y') }}</td>
            <td style="padding:3px 3px 3px 3%;">{{ $debt_act_1 }}</td>
			<td style="padding:3px;" class="center">{{ $receipt_detail->amount }}</td>
            <td style="padding:3px;" class="center"></td>
            @elseif($receipt_detail->payment_release_date == '0000-00-00' && $receipt_detail->payment_mode != 'cash')
            <td style="padding:3px;"></td>
            <td style="padding:3px;"></td>
            <td style="padding:3px 3px 3px 3%;">{{ $crdt_act_1 }} <br> {{ $receipt_detail->narration }}</td>
			<td style="padding:3px;" class="center"></td>
            <td style="padding:3px;" class="center">{{ $receipt_detail->amount }}</td>
            @endif
        </tr>
