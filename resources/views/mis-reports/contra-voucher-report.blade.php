...
<table class="comp-det" style="width:100%;">
		<tr>
			...
			<td>
                ...
                <h4>Contra Voucher Report<br> From {{\Carbon\Carbon::parse($start)->format('d-m-Y')}} to {{ \Carbon\Carbon::parse($end)->format('d-m-Y') }}</h4>
				</div>
...
<table border="0" style="width:100%;border-collapse:collapse;">
	...
	<tbody>
        @if(!empty($contra_list))
        <?php $amount=0; ?>
        @foreach($contra_list as $contra_detail)
    		<tr>
	            <td style="padding:3px;">{{ $contra_detail['voucher_no'] }}</td>
	            <td style="padding:3px;">{{ \Carbon\Carbon::parse($contra_detail['booking_date'])->format('d-m-Y') }}</td>
	            <td style="padding:3px 3px 3px 3%;">{{ $contra_detail['dr_account'] }}</td>
	  		    <td style="padding:3px;" class="center">{{ $contra_detail['dr_amt'] }}</td>
        ...
        <tr>
            ...
            <td style="padding:3px;"></td>
            <td style="padding:3px 3px 3px 3%;">{{ $contra_detail['cr_account'] }}</td>
            <td style="padding:3px;" class="center"></td>
            <td style="padding:3px;" class="center">{{ $contra_detail['cr_amt'] }}</td>
        </tr>
        <tr>
            <td style="padding:3px;"></td>
            <td style="padding:3px;"></td>
            <td style="padding:3px 3px 3px 3%;">Narration :({{ $contra_detail['narration'] }})</td>
            <td style="padding:3px;" class="center"></td>
            <td style="padding:3px;" class="center"></td>
        </tr>
        <?php $amount+=$contra_detail['cr_amt'];  ?>
      @endforeach

      	<tr>
			<td style="font-size:12px;text-align:left;padding-top: 10px;" colspan="3">Total</td>
			<td style="font-size:12px;text-align:center;padding-top:10px;border-top: 2px solid #000;"><?php echo $amount;?></td>
			<td style="font-size:12px;text-align:center;padding-top:10px;border-top: 2px solid #000; "><?php echo $amount;?></td>
		</tr>
